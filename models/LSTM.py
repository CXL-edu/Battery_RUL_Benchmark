...
class Model(ModelBasic):
    def __init__(self, configs):
        assert all([k in configs for k in ['seq_len', 'pred_len', 'in_dim', 'out_dim', 'batch_size',
                                           ...
                                           ]]), 'LSTM: Parameters are not completed!'
        ...
        self.c0 = nn.Parameter(torch.randn(configs.n_layer * (2 if configs.bidirect else 1), 1, configs.hidden_dim))
        self.lstm = nn.LSTM(input_size=configs.in_dim, hidden_size=configs.hidden_dim, num_layers=configs.n_layer,
    ...
    def forward(self, x):
        ...
        output = []
        h0, c0 = self.h0.repeat(1, x.size(0), 1), self.c0.repeat(1, x.size(0), 1)
        (output_temp, (hn, cn)) = self.lstm(x, (h0, c0))
        output.append(self.fc(output_temp[:, -1:, :]))
        for i in range(1, self.pred_len):
            (output_temp, (hn, cn)) = self.lstm(output[-1], (hn, cn))
            output.append(self.fc(output_temp[:, -1:, :]))
